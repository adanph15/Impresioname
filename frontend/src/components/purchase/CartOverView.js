    ...
    return (
        <div className="flex justify-center">
            <div className="w-11/12">
                <div className="flex flex-col min-[550px]:gap-6 py-6 ">
                    {cartItems.map((item) => (
                        <div key={item.id} className="flex items-center flex-col min-[550px]:flex-row gap-3 min-[550px]:gap-6 w-full max-xl:justify-center max-xl:max-w-xl max-xl:mx-auto pb-4 border-b border-gray-200">
                            <div className="img-box"><img src={item.filename} alt={item.name} className="xl:w-[140px] rounded-lg" /></div>
                            <div className="pro-data w-full max-w-sm">
                                ...
                                <button onClick={() => handleDeleteItem(item.id)} className="mt-2 px-4 py-2 bg-red-500 text-white rounded-md"><TrashIcon className="w-6"/></button>
                            </div>
                        </div>
                    ))}
                </div>

                <div className="bg-gray-50 rounded-xl p-6 w-full mb-6 mt-6 max-lg:max-w-xl max-lg:mx-auto">
